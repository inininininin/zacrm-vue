...
export default {
  ...
  mounted() {
    ...
    ts.$refs.ktTelephone.contentWindow.userWid =
      ts.$store.state.loginRefresh.token;
    ts.$refs.ktTelephone.contentWindow.callRecordUrl =
      "http://192.168.2.100/crm/kt-telephone/call-record";
    ts.$refs.ktTelephone.contentWindow.recordFileUrl =
      "http://192.168.2.100/crm/kt-telephone/record-file";
    ts.$refs.ktTelephone.contentWindow.heartBeatUrl =
      "http://192.168.2.100/crm/kt-telephone/heart-beat";
  },
  created() {
    // debugger;
    let thisVue = this;
    ...
    window.setInterval(() => {
      if (localStorage.getItem("currTel")) {
        ...
        $("#phoneNow p").html(
          localStorage.getItem("currTel") + "正在通话中. . ."
        );
      } else {
        $("#phoneNow").css({
          display: "none"
