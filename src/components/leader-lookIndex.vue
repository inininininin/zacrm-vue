...
<script>
  ...
  activated() {
    ...
    methods: {
      ...
      getDataNumber() {
        this.$axios.get(this.$Interface+'/ling-dao/user-list-sum?' + qs.stringify({
            userId: localStorage.getItem('id'),
          }))
          .then(res => {
            if (res.data.codeMsg) {
      ...
      async getCustomerWorkerTraceDay(_time){
        await this.$axios.get(this.$Interface+'/ling-dao/customer-worker-trace/customer-worker-trace-list-sum-by-day?' + qs.stringify({
          dayTime : _time,
          userId: localStorage.getItem('id'),
        }))
        .then(res => {
      ...
      traceNumber() {
        this.$axios.get(this.$Interface+'/ling-dao/customer-worker-trace/customer-worker-trace-list-sum?' + qs.stringify({
            userId: localStorage.getItem('id'),
          }))
          .then(res => {
            if (res.data.codeMsg) {
