      ...
      <div class="topselect">
        ...
        <div class="selectOption" style="width: 100%;height: auto;">
          ...
          <!-- 回访时间 : <input style="width:170px" @change="
							...
							lastPageNo()
					" type="date" /> -->
          <el-date-picker v-model="dataValue" type="date" placeholder="选择日期时间" align="right" :picker-options="pickerOptions"
            @change="elmentDataFn">
          </el-date-picker>
  ...
  export default {
          ...
          shortcuts: [{
            ...
            },
            {
              ...
              onClick (picker) {
                ...
                date.setTime(new Date(date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate() +
                  ' 00:00:00').getTime() + 3600 * 1000 * 24 * 3);
            ...
            },
            {
              ...
              onClick (picker) {
                const date = new Date();
                date.setTime(new Date(date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate() +
                  ' 00:00:00').getTime() + 3600 * 1000 * 24 * 7);
                picker.$emit('pick', date);
              }
            },
            {
              ...
              onClick (picker) {
                const date = new Date();
                date.setTime(new Date(date.getFullYear() + '-' + (date.getMonth() + 2) + '-' + date.getDate() +
                  ' 00:00:00').getTime());
                picker.$emit('pick', date);
              }
            }
    ...
    activated () {
      ...
      if (this.query != JSON.stringify(this.$route.query)) {
        ...
        // 		}
        // 	})
        $('#index .tbody').off('click', '.shoujiDiv div:first-child img').on('click', '.shoujiDiv div:first-child img',
          function () {
            console.log('s');
            console.log($(this).parent().parent().parent().attr('linkName'));
            if ($(this).parent().parent().parent().attr('tel') == '' || $(this).parent().parent().parent().attr('tel') ==
              null || $(this).parent().parent().parent().attr('tel') == undefined) {} else {
              $('#inp_send').val($(this).parent().parent().parent().attr('tel')).attr('linkName', $(this).parent().parent()
                .parent().attr('linkName'));
              $('.phoneNumber').html($(this).html());
              // console.log($('#inp_send').val())
              localStorage.setItem('tel', $('#inp_send').val());
              $('#btn_conn').click();
        ...
        $('#index .tbody').off('click', '.shoujiDiv div:last-child img').on('click', '.shoujiDiv div:last-child img',
          function () {
        ...
        $('#index .tbody').off('click', '.shoujiDiv1 div:first-child img').on('click',
          '.shoujiDiv1 div:first-child img',
          function () {
            console.log('s');
            console.log($(this).parent().parent().parent().attr('linkName'));
            if ($(this).parent().parent().parent().attr('tel') == '' || $(this).parent().parent().parent().attr('tel') ==
        ...
        $('#index .tbody').off('click', '.shoujiDiv1 div:last-child img').on('click', '.shoujiDiv1 div:last-child img',
          function () {
            thisValue.$axios.post('/push-call', qs.stringify({
                tel: $(this).parent().parent().parent().attr('tel'),
                name: $(this).parent().parent().parent().parent().children().eq(3).html()
    ...
    methods: {
      ...
      lastPage (pn, ps, kw, nature, area1Id, area2Id, area3Id, urgent, level) {
        ...
        $.ajax({
          ...
          data: 'kw=' + kw + '&level=' + level + '&pn=' + pn + '&ps=' + ps + '&nature=' + nature + '&area1Id=' +
            area1Id +
            '&area2Id=' + area2Id + '&area3Id=' + area3Id + '&urgent=' + urgent + '&toRevisitTimeFrom=' + thisValue
            .toRevisitTimeFrom +
            '&toRevisitTimeTo=' + thisValue.toRevisitTimeTo + '&paiBanCustomerWorkerHas=' + thisValue.paiBanCustomerWorkerHas +
            '&paiBanCustomerWorkerPhoneHas=' + thisValue.paiBanCustomerWorkerPhoneHas +
            '&zhuRenCustomerWorkerHas=' + thisValue.zhuRenCustomerWorkerHas + '&zhuRenCustomerWorkerPhoneHas=' +
            thisValue.zhuRenCustomerWorkerPhoneHas,
          async: true,
          success: function (res) {
            console.dir(res);
              ...
              if (res.data.itemList && res.data.itemList.length > 0) {
                for (var i in res.data.itemList) {
                  ...
                  if (res.data.itemList[i].paiBanCustomerWorkerPhone1) {
                    tel = res.data.itemList[i].paiBanCustomerWorkerPhone1.substring(0, 4) + '***' + res.data.itemList[
                      i].paiBanCustomerWorkerPhone1.substring(8, res.data.itemList[i].paiBanCustomerWorkerPhone1
                      .length);
                  }
                  if (res.data.itemList[i].tel) {
                    tel1 = res.data.itemList[i].tel.substring(0, 4) + '***' + res.data.itemList[i].tel.substring(
                      8, res.data.itemList[i].tel.length);
                  }
                  ...
                  $('#index .tbody').append(
                    ...
                    '<td class="enterHos">' +
                    '<a href="#/add-hos?id=' + res.data.itemList[i].customerId + '">' + (res.data.itemList[i].name ||
                      '') + '</a>' +
                    '</td>' +
                    '<td  linkName="' + (res.data.itemList[i].name || '') + '" tel="' + (res.data.itemList[i].tel ||
                      '') + '">' +
                    '<div style="display:inline-block;width: 100px;" Title="' + res.data.itemList[i].tel + '">' +
                    (tel1 || '') + '</div>' +
                    ...
                    '<td  linkName="' + (res.data.itemList[i].name || '') + '" tel="' + (res.data.itemList[i].paiBanCustomerWorkerPhone1 ||
                      '') + '">' +
                    '<div style="display:inline-block;width: 100px;" Title="' + res.data.itemList[i].paiBanCustomerWorkerPhone1 +
                    '">' + (tel || '') + '</div>' +
                    (tel
                      ? '<div class="shoujiDiv">' +
      ...
      lastPageNo () {
        ...
        $.ajax({
          ...
          data: 'kw=' + thisValue.kw + '&level=' + thisValue.level + '&nature=' + thisValue.nature + '&area1Id=' +
            thisValue.area1Id + '&area2Id=' + thisValue.area2Id +
            '&area3Id=' + thisValue.area3Id + '&urgent=' + thisValue.urgent + '&paiBanCustomerWorkerHas=' +
            thisValue.paiBanCustomerWorkerHas + '&paiBanCustomerWorkerPhoneHas=' + thisValue.paiBanCustomerWorkerPhoneHas +
            '&zhuRenCustomerWorkerHas=' + thisValue.zhuRenCustomerWorkerHas + '&zhuRenCustomerWorkerPhoneHas=' +
            thisValue.zhuRenCustomerWorkerPhoneHas,
          async: true,
          success: function (res) {
            if (res.code == 0) {
              ...
              $('#index #box').paging({
                ...
                callback: function (page) { // 回调函数
                  ...
                  var nature = $('#index .nature').val();
                  thisValue.pn = page;
                  thisValue.lastPage(page, thisValue.ps, thisValue.kw, nature, thisValue.area1Id, thisValue
                    .area2Id, thisValue.area3Id, thisValue.urgent, thisValue.level);
                }
              });
            }
          }
