  ...
  <div id="login" class="login">
    ...
    <div id="updatePwdPad" style="display:none;">
      ...
      <div class="number oldpwd" style="margin-top: 24px">
        ...
        <div>
          <img src="../assets/img/mima.png" alt="" /> <span>旧密码</span>
        </div>
      </div>
      <div class="number newpwd" style="margin-top: 24px">
        <input autocomplete="off" class="passwords1" type="password" />
    ...
    >
      <div style="height: 50px;box-sizing: border-box;width: 100%;">
        <router-link
          :to="{ path: '/index-new', query: 'time: new Date().getTime()' }"
          replace
        >
          <div
            class="typeDialogClass"
            @click="dialogTableVisible = false"
            :style="{ width: admin && statistician ? '33.3%' : '50%' }"
          >
            用户端
          </div>
        </router-link>
        <router-link
          :to="{ path: '/leader-index', query: 'time: new Date().getTime()' }"
          replace
          v-if="admin"
        >
          <div
...
export default {
  ...
  activated() {
    ...
    if (_this.$store.state.loginRefresh) {
      ...
      if (
        _this.$store.state.loginRefresh.statisticia != 1 &&
        _this.$store.state.loginRefresh.level < 2
      ) {
