<template>
  <div id="index" class="index">
    ...
    <router-link
      :to="{ path: 'index', query: { time: new Date().getTime() } }"
    ...
    <div class="mainbox">
      <div class="topselect">
        <div style="width: 100%;height: auto;">
          ...
          <span style="font-size: 17px;margin-left: 7px;" title="分机号"
            >({{ $store.state.loginRefresh.extTel }})</span
          >
          <router-link
            :to="{ path: '/add-hosNew', query: { time: new Date().getTime() } }"
            class="addHos"
          >
            新增医院
          </router-link>
          ...
          <a
            href="javascript:;"
            class="loginout"
            style="float: right;line-height: 70px;margin-left: 20px;"
            >退出登录</a
          >
          ...
          <span
            class="lastHis"
            style="float: right;line-height: 70px;margin-right: 20px;"
            >上次浏览记录</span
          >
        </div>
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <a @click="jumbDetail(scope.row.customerId)" href="javascript:">{{
                scope.row.name
              }}</a>
              <!-- <a target="_blank" 
    ...
    <div class="seccion">
      ver : {{ $version }} ser : {{ $store.state.serVersion }}
    </div>
  </div>
</template>
<script>
...
export default {
    ...
    if (this.query != JSON.stringify(this.$route.query)) {
      ...
      $("#index table").on("click", "tr .enterHos", function() {
        ...
        $.ajax({
          ...
          data:
            "name=" + $(".peoname").html() + "&value=" + JSON.stringify(param),
          async: true,
          success: function(res) {
            if (res.code == 0) {
            }
      ...
      // 	})
      $("#index .tbody")
        .off("click", ".shoujiDiv div:first-child img")
        .on("click", ".shoujiDiv div:first-child img", function() {
          console.log("s");
          console.log(
            $(this)
              .parent()
          ...
          if (
            ...
            thisValue.$store.state.telTimeMIntenSeconds = 0;
            $(".phoneEnd_num").html(
              thisValue.$store.state.telTimeMIntenSeconds + " s"
            );
          }
        });
      $("#index .tbody")
        .off("click", ".shoujiDiv div:last-child img")
        .on("click", ".shoujiDiv div:last-child img", function() {
          let telNow = "";
          if (
            $(this)
              .parent()
      ...
      $("#index .tbody")
        ...
        .on("click", ".shoujiDiv1 div:first-child img", function() {
  ...
  methods: {
    ...
    elmentDataFn(_value) {
      ...
      console.log(
        this.moment(this.moment(_value).valueOf()).format("YYYY-MM-DD HH-mm-ss")
      );
      let time = this.moment(this.moment(_value).valueOf())
        .format("YYYY-MM-DD HH-mm-ss")
        .split(/[ ]+/);
      time = time[0].replace(/-/g, "/") + " 00:00:00";
      let nowTime = time;
      if (nowThis.nowTimeData) {
        nowTime = this.moment(this.moment(new Date().getTime()).valueOf())
          .format("YYYY-MM-DD HH-mm-ss")
          .split(/[ ]+/);
      ...
      if (_value) {
        ...
        this.toRevisitTimeTo =
          this.moment(time).valueOf() + (1 * 24 * 60 * 60 * 1000 - 1);
        console.log(
          "toRevisitTimeFrom" +
            this.moment(this.toRevisitTimeFrom).format("YYYY-MM-DD HH-mm-ss")
        );
        console.log(
          "toRevisitTimeTo" +
            this.moment(this.toRevisitTimeTo).format("YYYY-MM-DD HH-mm-ss")
        );
      } else {
            ...
            if (res.data.itemList) {
              for (var i in res.data.itemList) {
                if (res.data.itemList[i].lastCustomerWorkerTrace) {
                  res.data.itemList[
                    i
                  ].lastCustomerWorkerTrace = thisValue.getDateDiff(
